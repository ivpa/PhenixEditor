...
namespace DataEditorX
{
	public partial class DataEditForm : Form
	{
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                int num = lv_cardlist.FocusedItem.Index+1;
                if (this.lv_cardlist.Items.Count>num)
                {
                    this.lv_cardlist.Items[num].Selected = true;
                    int index =(page - 1) * MaxRow+ num;
                    if (index < lv_cardlist.Items.Count)
                    {
                        Card c = cardlist[index];
                        SetCard(c);
                    }
            ...
            else if (this.lv_cardlist.Items.Count > 0)
            { 
                this.lv_cardlist.Items[0].Selected = true;
              //  this.lv_cardlist.SelectedItems.[0] = true;
                this.lv_cardlist.FocusedItem = this.lv_cardlist.Items[0];
                int index = (page - 1) * MaxRow;
                if (index < cardlist.Count)
                {
                    Card c = cardlist[index];
                    SetCard(c);
          ...
          //  this.lv_cardlist.DoubleClick(null, null);
            /* if (lv_cardlist.SelectedItems.Count > 0)
             {
                 int sel = lv_cardlist.SelectedItems[0].Index;
                 int index = (page - 1) * MaxRow + sel;
